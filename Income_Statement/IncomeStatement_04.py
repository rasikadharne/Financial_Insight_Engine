#Hardcoded values are removed as the XBRL tags are now updated to reflect accurate line item 
...
def extract_income_data(xbrl_data):
    ...
    # Print all available tags for debugging (optional, can remove later)
    # for tag in xbrl_data:
    #     print(f"Available tag: {tag}")

    # Derive Transaction-based Revenues, Other Revenues, Tech and Dev, Operations
    # Note: These derivations reference labels not in income_tags (e.g., "Net Interest Revenues").
    # Since commented tags are ignored, we'll skip unavailable derivations unless you add them back.
    for year in income_data.get("Total Net Revenues", {}):
        total_revenues = income_data["Total Net Revenues"].get(year, 0)
        # Skipping "Net Interest Revenues" and "Other Revenues" as they’re commented out
        total_op_exp = income_data["Total Operating Expenses"].get(year, 0)
        brokerage = income_data["Brokerage and Transaction"].get(year, 0)
        # "Technology and Development" isn’t in tags, using R&D as proxy if intended
        tech_dev = income_data["ResearchAndDevelopmentExpense"].get(year, 0)  # Adjusted
        marketing = income_data["Marketing"].get(year, 0)
        g_and_a = income_data["General and Administrative"].get(year, 0)
        
        # Derive Operations (adjusted for available tags)
